...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['firstname'])){
            ...
            $errors[] = "Nombre de usuario no puede ser inferior a 2 o máyor de 64 caracteres";
        } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_name'])) {
            $errors[] = "Nombre de usuario no es cmpatible: Sólo aZ y los números están permitidos , de 2 a 64 caracteres";
        } elseif (empty($_POST['user_email'])) {
            $errors[] = "El correo electrónico no puede estar vacío";
        } elseif (strlen($_POST['user_email']) > 64) {
            $errors[] = "El correo electrónico no puede ser superior a 64 caracteres";
        } elseif (!filter_var($_POST['user_email'], FILTER_VALIDATE_EMAIL)) {
            $errors[] = "La dirección de correo electrónico no está en un formato de correo electrónico válido";
        } elseif (
		...
		$query_check_user=mysqli_num_rows($query_check_user_name);
                if ($query_check_user == 1) {
                    ...
                    if ($query_new_user_insert) {
                        ...
                        $errors[] = "Lo sentimos , el registro falló. vuelva a intentarlo.";
                    }
                }
